...
public class Projection {
   ...
   */
  public void getMetersPerPixelAtLatitude(@FloatRange(from = -90, to = 90) final double latitude, final Callback<Double> listener) {
    mapState.queueRenderEvent(new MapRunnable() {
      @Override
      public void execute(@NonNull NativeMapView nativeMapView) {
  ...
  public void getVisibleRegion(final Callback<VisibleRegion> listener) {
    mapState.queueRenderEvent(new MapRunnable() {
      ...
      public void execute(@NonNull NativeMapView nativeMapView) {
        ...
        final VisibleRegion visibleRegion = new VisibleRegion(topLeft, topRight, bottomLeft, bottomRight, builder.build());
        mapState.queueUiEvent(new Runnable() {
          @Override
          public void run() {
